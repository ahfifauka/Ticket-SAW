...
import KaryawanForm from "./_components/TeknisiForm"
...
const Index = ({ auth }: PageProps) => {
  const { teknisi } = usePage<CustomPageProps>().props
  const { delete: destroy } = useForm()
  const handleDelete = (id: number) => {
    destroy(route("teknisi.destroy", id), {
      preserveScroll: true,
    })
  }
  return (
            ...
            <div className="flex justify-between items-center gap-x-3 mt-5 px-10">
              <div>
                <small className="text-muted-foreground">
                  Page {teknisi.meta.current_page} from {teknisi.meta.last_page}
                </small>
              </div>
              <div className="flex gap-x-3">
                {teknisi.meta.links.map((link, index) => {
                  if (link.label === "&laquo; Previous" || link.label === "Next &raquo;") {
                    return null
                  }
                  return (
                    <Link key={index} href={link.url ? link.url : ""} className={`border px-3 py-2 rounded-md aspect-square ${teknisi.meta.current_page.toString() === link.label ? "bg-stone-900 text-white" : ""}`}>
                      {link.label}
                    </Link>
                  )
                })}
